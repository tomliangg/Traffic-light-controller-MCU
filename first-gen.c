...
int main(void) {
  ...
  GPIO_Init(GPIOD, &GPIO_InitStructure);
	...
	while (1) {
			...
			/* Insert delay */
			Delayms(0x1770+AddTime_EW);   //0x1770 in hex is equivalent to 6000 in decimal
			...
			if (counter_NS_pedestrian == 0x00)   //if nobody press the pedestrian cross button
			{
				GPIO_SetBits(GPIOD, GPIO_Pin_2);    //turn on red pedestrian light
				GPIO_ResetBits(GPIOD, GPIO_Pin_0); //turn off the green pedestrian light
			}
			else {                                //if someone press the cross button
			...
			counter_NS_pedestrian=0x00;   //reset this value
